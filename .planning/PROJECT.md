...
**v2.1 progress:** Phase 19 Cross-Block Highlights — SHIPPED 2026-08-31 (5 plans). Endpoint-composed span capture: one global TextPositionSelector built from two per-endpoint block resolutions (D5-06 single-block vocabulary fully retired), with a boundary refusal taxonomy that rejects ineligible endpoints whole and calm with a verbatim reader-visible reason; render coverage in both modes — scrolling block-local fragments and paginated cross-fragment slicing share one slicer, with per-item list threading (nested lists included), figure-caption and code marks, and exactly one DOM id per highlight per mounted page (`HighlightSlice.isFirst` scrolling, `seenHighlightIds` paginated); `firstFragmentExcerpt` (structural honesty booleans, never content-sniffing) adopted at every quote.excerpt surface; multi-line Markdown export with per-line escaping. ANNO-08..12 verified 5/5 via the 21-cell eligibility matrix (8 kinds × crossings × gap classes × refusal classes) on chromium/firefox/webkit plus durability (typography/mode/reload/export-import/review-jump) and atomic delete/note-edit cells. Full suite at gate: unit 1416 + e2e 1581 passed / 0 failed / exit 0 in one invocation, fresh dev server, no engine/subset filtering (19-05 honest record).

**v2.1 progress:** Phase 20 Safe Local Image Fidelity — SHIPPED 2026-08-31 (verification 5/5). Meaningful source figures, alt text, and captions preserved through ingestion into the canonical sanitized model; secondary image assets fetched through controlled SSRF-safe handling (redirect/address/media-type/byte/pixel/count/animation/decode limits, corpus-calibrated); saved articles never re-contact third-party hosts (controlled local assets with explicit lifecycle/deletion); assets round-trip export/import with bundle limits and no broken references; semantic figure rendering with stable intrinsic geometry and calm broken/unsupported fallbacks in both modes; load/decode/failure/size changes never silently clip, duplicate, omit, reorder, or destabilize pagination (canonical location preserved through required repagination). IMG-01..06 Complete.

**v2.1 progress:** Phase 21 Integrated Refinement and Acceptance — SHIPPED 2026-09-07 (8 plans incl. 2 gap-closure waves; verification 5/5, UAT resolved 7/7). Truthful reading-width slider over real min/max bounds (POLISH-09), tag popover anchored to its trigger (POLISH-08), Highlights destination cohesion with quiet jump glyphs + token conformance (POLISH-10), Impeccable-informed audit with a deferred-items ledger (POLISH-11), one integrated end-to-end spine spec over the v2.1 core flow (ACPT-07), and ACCEPTANCE-PROTOCOL v1.3 with the acceptance edge-matrix (ACPT-08). Gap closures: WebKit button-turn focus-handoff self-heal (never-body contract, live VoiceOver+Safari checkpoint user-approved) and openToc open-focus race hardening with the honest gate re-established on a fresh dev server. Milestone complete at 40/40 requirements; closing gate: lint clean, unit 1605/0/13, e2e 1733/0/20 in one fresh-server invocation.
...
- ✓ Cross-block highlights with durable, honest anchors (ANNO-08..12) — one highlight captured from a selection spanning eligible mounted blocks (endpoint-composed global selector), rendering across blocks in both modes as one identity/one range/one optional note (exactly one DOM id per mounted page), staying attached through repagination, mode/typography changes, reopening, review navigation, and export/import, managed atomically (delete/note-edit leave no fragments), with ineligible boundaries refused whole and explained from the supported-content matrix; verified 5/5 via the 21-cell × 3-engine eligibility matrix plus durability and atomicity cells. — v2.1 (Phase 19)
- ✓ Safely ingested source images and captions (IMG-01..06) — figures/alt/captions preserved into the canonical sanitized model, SSRF-safe controlled secondary fetches with corpus-calibrated limits, local-first assets that never re-contact third-party hosts, export/import round-trip with bundle limits and no broken references, semantic rendering with stable intrinsic geometry and calm fallbacks in both modes, and load/decode/failure behavior that never silently destabilizes pagination; verified 5/5. — v2.1 (Phase 20)
- ✓ Interface refinement and full-matrix acceptance (POLISH-08..11 + ACPT-07/08) — truthful reading-width bounds, trigger-anchored tag popover, cohesive Highlights destination, Impeccable-informed audit with deferred ledger, one integrated end-to-end spine spec, and ACCEPTANCE-PROTOCOL v1.3 acceptance edge-matrix with the WebKit focus-handoff self-heal human-approved (VoiceOver+Safari) and the honest full gate green on a fresh server; verified 5/5, UAT resolved 7/7 — milestone 40/40. — v2.1 (Phase 21)

### Active

None — v2.1 Reader Experience milestone requirements are all validated.
...
| Cross-block capture composes two per-endpoint block resolutions into one global TextPositionSelector; ineligible endpoints refuse whole with a verbatim reason | Endpoint composition keeps one identity/one range/one note while the refusal taxonomy preserves the no-silent-garbage promise — narrowing silently would re-attach to guessed content | ✓ Good — Phase 19: 21-cell eligibility matrix × 3 engines, refusals verbatim-asserted |
| Exactly one DOM id per highlight per mounted page (`HighlightSlice.isFirst` scrolling; `seenHighlightIds` paginated) | A highlight split across fragments/blocks must remain one addressable identity — duplicate ids break fragment navigation, missing ids break review jumps | ✓ Good — Phase 19: exactly-one id asserted per mounted page in both modes |
...
---
*Last updated: 2026-09-07 after completing Phase 21 (Integrated Refinement and Acceptance) — final phase of the v2.1 Reader Experience milestone (8/8 phases, 40/40 requirements)*
