...
**Domain:** Accessible browser-based long-form reader with responsive pagination
**Researched:** 2026-07-26
**Confidence:** HIGH for the application/tooling stack; MEDIUM for Pretext.js integration because it is pre-1.0 and must be validated against browser layout
...
**Recommendation: adopt as an experimental measurement primitive, not as the pagination engine. Pin `@chenglou/pretext@0.0.8` exactly and isolate it behind an adapter.**

Pretext's official API fits one expensive inner loop: `prepare(text, font, options)` performs segmentation and canvas measurement once, while `layout(prepared, width, lineHeight)` and richer line-range APIs repeat arithmetic layout without DOM reads. That is valuable during viewport and typography changes.

It does not own the normalized document tree, semantic rendering, block fragmentation, image/caption sizing, code-block overflow, footnotes, annotation anchoring, font lifecycle, focus/navigation, or fallback policy. Its official README also states that automatic hyphenation is not built in; the rich-inline helper is intentionally limited to flat inline data with `white-space: normal`; and its cursors are segment/grapheme cursors rather than raw source offsets. Canvas-derived widths can diverge from final browser layout when CSS/font state is mismatched, so Pretext output must be treated as a prediction and guarded by rendered overflow checks.

Adoption gate:

1. Build a reference DOM measurer and the corpus test harness first.
2. Compare Pretext-predicted line counts and page boundaries against rendered results in Chromium, Firefox, and WebKit after `document.fonts.ready`.
3. Enable the fast path only for block/style/language combinations that meet the agreed error tolerance.
4. Fall back to DOM block measurement or scrolling mode on unsupported/risky content or detected overflow.

## Installation

```bash
# Runtime (pin Pretext exactly while it remains pre-1.0)
npm install react@19.2.8 react-dom@19.2.8 dexie@4.4.4 zod@4.4.3 @chenglou/pretext@0.0.8

# Build and types
npm install -D vite@8.1.5 @vitejs/plugin-react@^6 typescript@7.0.2 @types/react@^19 @types/react-dom@^19

# Tests and accessibility
npm install -D vitest@4.1.10 @testing-library/react@16.3.2 @testing-library/dom @testing-library/user-event @playwright/test@1.61.1 @axe-core/playwright@4.12.1
npx playwright install
```

Commit the lockfile. Treat the exact versions above as a researched baseline, then use routine dependency updates rather than floating runtime dependencies.
...
**For unsupported content or unstable measurement:**
...
---
*Stack research for: Lem Reader accessible pagination prototype*
*Researched: 2026-07-26*
