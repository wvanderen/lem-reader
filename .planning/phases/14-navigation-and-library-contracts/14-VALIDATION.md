...
| 14-02:2 | 14-02 | 2 | LIB-07, LIB-08 | — | counts derive from the same policy functions as membership (structural agreement); no `filter(...).length` shortcuts | e2e regression (chromium) | `npx vitest run tests/component/App.test.tsx && npx playwright test tests/e2e/library/happy-path.spec.ts --project=chromium` | ✅ (happy-path unmodified = the guard) | ⬜ pending |
| 14-02:3 | 14-02 | 2 | NAV-04 | T-14-05 | cold loads/reloads never move focus; h1 focus only on warm mounts + view switches | e2e regression (chromium) | `npx playwright test tests/e2e/library/happy-path.spec.ts tests/e2e/library/search-tag-filter.spec.ts --project=chromium` | ✅ | ⬜ pending |
