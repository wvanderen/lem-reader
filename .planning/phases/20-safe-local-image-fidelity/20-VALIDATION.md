...
> Populated from plan `<verify>` blocks + 20-RESEARCH §Validation Architecture (revisions R1+R2, per checker fixes — includes the 20-04/20-08 split; R2: 20-03-T2 creates library-source.test.ts, 20-08-T1 closes the IMG-05 axe row inside refusal-matrix.spec).
...
| 20-03-T2 | 03 | 2 | IMG-03, IMG-04 | T-20-13, T-20-14 | One read-write transaction: asset-put failure rolls back the article put (saved article always complete); article/book delete cascades + re-ingest upsert range-delete all inside the same transaction (no orphan blobs) | unit | `npx vitest run tests/unit/persistence/assets-cascade.spec.ts tests/unit/library/library-source.test.ts tests/unit/persistence/books-store.test.ts` | ❌ W0 — assets-cascade.spec from T1; library-source.test.ts created by this task | ⬜ pending |
...
| 20-08-T1 | 08 | 5 | IMG-03, IMG-05, IMG-06 | T-20-05, T-20-15 | Non-vacuous offline proof: route-abort guard registered pre-navigation + asserted-empty external-request array (any accidental fetch fails); page-count identity across all img load/error events + no fallback banner; 5 formats × 3 engines decode cells; refusal placeholder + caption-mark cells in both modes; AxeBuilder scan (WCAG tags, a11y.spec.ts precedent) on the seeded placeholder state asserting zero serious/critical violations — closes the RESEARCH IMG-05 "axe on placeholder" row inside refusal-matrix.spec (post-regen no swept fixture renders a placeholder, so a11y.spec.ts is not extended) | e2e | `npx playwright test tests/e2e/imagery --project=chromium --project=firefox --project=webkit` | ❌ W0 — the four imagery specs created by this task | ⬜ pending |
...
- [ ] `tests/unit/library/library-source.test.ts` — save-signature default cells (assets omitted → zero asset rows; explicit assets → exact row count) — created by **20-03-T2** (revision R2: this file is new; existing DexieLibrarySource coverage lives in `tests/unit/ingestion-client.test.ts`, which 20-02-T3 owns this wave — no same-wave conflict)
...
*All phase behaviors have automated verification.* The placeholder surface copy ("Image unavailable.") is asserted verbatim in 20-04-T2 acceptance, exercised by 20-08-T1 refusal cells, and axe-scanned (AxeBuilder, WCAG tags, zero serious/critical) inside refusal-matrix.spec — the RESEARCH §Validation Architecture IMG-05 "axe on placeholder (a11y.spec — extend)" row is satisfied by that cell rather than by extending a11y.spec.ts, because post-regen no swept fixture renders the placeholder (figure-heavy is the sole figure fixture and both its figures convert to accepted assets); the seeded refusal-matrix state is the only reader-facing placeholder surface. No new UI interaction pattern is introduced beyond the project baseline (20-UI-SPEC §Regression Targets lists no phase-20-specific manual row).
...
**Approval:** approved 2026-08-31 (planner sign-off, revision R2 — populated from plan verify blocks + 20-RESEARCH §Validation Architecture per checker fixes; R2 task-created scaffold count is now 9, matching the 9 ❌ W0 rows)
